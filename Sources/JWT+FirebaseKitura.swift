...
extension JWT {
  ...
  private func time(with key: String) -> Date? {
    guard let interval = claims[key] as? Double else { return nil }
  ...
  var emailVerified: Bool? { return claims["email_verified"] as? Bool }
  
  func verifyAlgorithm() throws {
    let alghorithm = "RS256"
    let algorithmName = header[.alg] as? String ?? ""
